...
describe('activityLogger', () => {
    ...
    describe('helper functions', () => {
        it('logLogin should call logActivity with correct params', async () => {
            vi.mocked(supabase.auth.getUser).mockResolvedValue({ data: { user: { id: 'u1' } }, error: null });
            const selectMock = vi.fn().mockResolvedValue({ data: [], error: null });
            vi.mocked(supabase.from).mockReturnValue({ insert: vi.fn().mockReturnValue({ select: selectMock }) });
